 ...
 */
 class Perfume extends Item implements SecureItem
 {
    ...
    /*
     * TODO: Constructor that takes and sets name, expiry days, price. Sets locked to True.
     */
    public Perfume(String name, int expiryDays, double price)
    {
        this(name);
        this.expiresInDays = expiryDays;
        this.price = price;
    ...
    /*
     * TODO: Removing a security tag. If item is not locked, process fails. Otherwise succeeds. Returns success value
     * based on whether lock is successfully removed.
     */
...
class EasterSale implements OnSale
{
    ...
    final double salePercent; // sale percentage
      ...
      /*
       * Constructor that takes and sets a sale percentage and minimum amount for sale
       * to qualify. The percent is in an 'easy form' i.e. as a percentage like 7.5 or
       * 25. This needs to be converted to 'normal form' which is a multiplier e.g.
       * 0.075 for 7.5% and 0.25 for 25%. The constructor also sets the message based
       * on minimum amount and the sale percent as "Spend over M, Get P off for Easter!"
    ...
    EasterSale(double salePercent, double minimumAmount)
    {
      ...
      /*
       * Implementation of OnSale reduction Takes the total amount and returns the
       * reduced amount based on applying sale percent discount
       */
    public double reduction(double totalAmount)
    {
      ...
      /*
       * SaleCondition that take in an array of items and checks whether the total is
       * greater than minimumAmount, and returns a success value reflecting this.
       */
    public boolean saleCondition(Item[] items)
    {
      ...
      /*
       * toString that returns the sale message
       */
    public String toString()
    {
        return this.message;
    ...
    public static double billItems(Item[] items, OnSale sale)
    {
        ...
        for(Item item: items)
        {
            ...
            if(item instanceof Milk)
            {
                ...
                {
                    messageStorage = product.storageProcedure();
                    System.out.println(item.name + " (" + item.price + ") " + "(Storage: " + messageStorage + ")");
                }
            }
